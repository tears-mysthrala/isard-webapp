...
def get_folder_for_machine(vm_id):
    """Obtiene la carpeta a la que pertenece una máquina."""
...
def get_machines_without_folder():
    """Obtiene las máquinas que no están en ninguna carpeta."""
...
def get_machines_in_folder(folder_id):
    """Obtiene las máquinas que están en una carpeta específica."""
...
def fetch_vms():
    ...
    try:
        ...
        for vm in data:
            ...
            MAQUINAS[str(vm["id"])] = {
                "id": vm["id"],
                "name": vm.get("name", f"Máquina {vm['id']}"),
...
def api_action(vm_id, action):
    """
    Realiza una acción (start/stop) usando el método GET y la ruta invertida.
...
def get_status_html(status):
    """Devuelve el HTML formateado según el estado."""
...
<body>
    <div class="container">
        ...
        <div class="folder-section">
            <div class="folder-header" data-bs-toggle="collapse" data-bs-target="#folder-{{ folder_id }}">
                <h4>
                    ...
                    <span class="badge bg-secondary folder-badge ms-2" id="folder-{{ folder_id }}-count">{{ folder.machines|length }} máquinas</span>
                </h4>
                <div class="folder-actions" onclick="event.stopPropagation();">
                    ...
                    <a href="{{ url_for('delete_folder', folder_id=folder_id) }}" class="btn btn-sm btn-outline-danger" title="Eliminar carpeta" onclick="return confirm('¿Estás seguro de eliminar esta carpeta? Las máquinas no se eliminarán.');">
            ...
            <div class="folder-content collapse show" id="folder-{{ folder_id }}">
                <div class="row">
                    ...
                    {% else %}
                    <div class="col-12">
                        <p class="text-muted mb-0"><i class="fas fa-info-circle me-2"></i>Esta carpeta está vacía.</p>
                    </div>
                    {% endfor %}
                </div>
            </div>
        ...
        <div class="folder-section unassigned-section">
            <div class="folder-header" data-bs-toggle="collapse" data-bs-target="#unassigned-folder">
                <h4>
                    ...
                    <span class="badge bg-secondary folder-badge ms-2" id="unassigned-count">{{ unassigned_machines|length }} máquinas</span>
                </h4>
            </div>
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted">Las máquinas virtuales aparecerán aquí cuando estén disponibles.</p>
        </div>
        {% endif %}
    </div>
        ...
        async function refreshData() {
            try {
                ...
                for (const [folderId, folderData] of Object.entries(data.folders)) {
                    ...
                    if (badge) {
                        badge.textContent = `${Object.keys(folderData.machines).length} máquinas`;
                    }
                }
...
def assign_to_folder(folder_id, vm_id):
    ...
    save_folders()
    flash(f"Máquina asignada a '{FOLDERS[folder_id]['name']}'.", "success")
    return redirect(url_for("index"))
...
def unassign_from_folder(vm_id):
    ...
    for folder in FOLDERS.values():
        if vm_id in folder.get("machines", []):
            folder["machines"].remove(vm_id)
            save_folders()
            flash("Máquina removida de la carpeta.", "success")
            return redirect(url_for("index"))
    
    flash("La máquina no estaba en ninguna carpeta.", "info")
    return redirect(url_for("index"))
